...
async function toRemove() {
  try {
    ...
    for (const index in store.state.cart[0].attributes.glassesId.data)
      items.push(store.state.cart[0].attributes.glassesId.data[index].id);
    let diference = items
      .filter((x) => ![props.id].includes(x))
      .concat([props.id].filter((x) => !items.includes(x)));
  ...
  <div class="col-md-6 col-lg-4 col-xl-3">
    <div id="product-1" class="single-product">
      <div class="part-1 rounded">
        <ul>
          <li>
            <a @click="toRemove"><i class="fas fa-solid fa-trash"></i></a>
          </li>
        </ul>
      </div>
      <div class="part-2">
